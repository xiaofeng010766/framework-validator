...
public abstract class DefaultCustomValidator<T extends Serializable> implements BaseValidator {
     ...
     * @param required  是否必须
     * @param regex     正则表达式
     * @param params     正则表达式的解释
     */
    protected void validatorPattern(String sheetName, Integer rowNum, String fieldName, String value, boolean required, String regex,String params) {
        if (validatorInit(rowNum, value, fieldName, required)) {
            return;
        }
        if (!StringValidator.validatorPattern(value, regex)) {
            this.addErrorMessage(rowNum, fieldName, Validator.MESSAGE_REGEX, new String[]{regex,params}, sheetName);
        }
    }
     ...
     * @param required  是否必须
     * @param regex     正则表达式
     * @param regex     正则表达式的解释
     */
    protected void validatorPattern(Integer rowNum, String fieldName, String value, boolean required, String regex,String params) {
        validatorPattern(null, rowNum, fieldName, value, required, regex,params);
    }
     ...
     * @param params     正则表达式的解释
     */
    protected void validatorPattern(String fieldName, String value, boolean required, String regex,String params) {
        validatorPattern(null, fieldName, value, required, regex,params);
    }
    ...
    private boolean validatorInit(Integer rowNum, Object value, String fieldName, boolean required) {
        if (!required && null == value) {
            return true;
        } else if (!required && value.getClass().isArray() && ArrayUtils.isEmpty((Object[]) value)) {
            return true;
        } else if (!required && value instanceof String && StringUtils.isEmpty((String) value)) {
            return true;
        } else if (!required && value instanceof List && CollectionUtils.isEmpty((List) value)) {
            return true;
        } else if (!required && value instanceof Map && ((Map) value).size() == 0) {
            return true;
        }
